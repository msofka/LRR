            ...
            itk::Vector< float, 3 > const &  keypointNormal )
{
  ...
  unsigned int azimuthBin = (unsigned int)( vcl_floor( azFrac ) );
  azFrac -= (azimuthBin-.5);

  double elFrac = elevation_wrt_keypoint / m_ElevationBinSize;
  unsigned int elevationBin = (unsigned int)( vcl_floor( elFrac ) );
...
::GenerateData()
{
  ...
  for( unsigned int i = 0; i < keypointPoints->size(); ++i ) {
    // gather features in a neighborhood around keypoint
    //

    typename KeypointMeshType::PointType  keypointPoint;
    keypoints->GetPoint( i, &keypointPoint );
...
typename KeypointMeshType::Pointer       keypoints    = dynamic_cast< TInputMesh1 * >( this->GetInput(0) );
typename KeypointMeshType::PointDataContainerPointer  pointDataOld = keypoints->GetPointData();
keypoint_direction = (*pointDataOld)[i].m_Directions[0];
keypoint_bidirection = (*pointDataOld)[i].m_Directions[1];
//std::cout << "INSIDE DESC: " << keypoint_direction << "   " << keypoint_bidirection << std::endl;


    //keypoint_direction[0] = 1.0;
    //keypoint_direction[1] = 0.0;
    //keypoint_direction[2] = 0.0;

    vnl_vector< float >  descriptor( 195, 0.0f );

    DescriptorAttribute  attribute( keypoint_direction, keypoint_bidirection, descriptor );
    m_KeypointsWithDescriptors[0].push_back( keypointPoint );
    m_KeypointsWithDescriptorsData[0].push_back( attribute );
                              ...
                              int threadId )
{
  ...
  for( KeypointIteratorType  it = startIterator; it != endIterator; ++it, ++count ) {
    // gather features in a neighborhood around keypoint
    //
...
typename KeypointMeshType::Pointer       keypoints    = dynamic_cast< TInputMesh1 * >( this->GetInput(0) );
typename KeypointMeshType::PointDataContainerPointer  pointDataOld = keypoints->GetPointData();
keypoint_direction = (*pointDataOld)[count].m_Directions[2];
keypoint_bidirection = (*pointDataOld)[count].m_Directions[1];

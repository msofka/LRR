...
cdcl_obj_fun_par_transfer<dimen, dof>::cdcl_obj_fun_par_transfer( vcl_vector< match_sptr_type > const &  matches,
...
template < unsigned int dimen, unsigned int dof >
void cdcl_obj_fun_par_transfer<dimen, dof>::compute( const vnl_vector< double > &  x,
                                                     double*                       f,
                                                     vnl_vector< double >*         g )
{
  ...
  vcl_vector< vnl_matrix_fixed< double, dimen, dimen > > const &  Mkls = trans_->jacobian_of_Jp();
  ...
  for( typename vcl_vector< match_sptr_type >::size_type  i = 0; i < matches_.size(); ++i ) {
    ...
    // transfer error covariance (estimated) and mapped covariance of a moving point
    vnl_matrix_fixed< double, dimen, dimen >  Cij_p = trans_->get_covarianceJ() + Jp * p->covariance_ * Jp.transpose();
    ...
    for( typename vcl_vector< match_sptr_type >::size_type  j = 0; j < matches_[i]->to_.size(); ++j ) {
      ...
      // matching error
      vnl_vector_fixed< double, dimen >  const &  err_vec = trans_->map_loc( p->location_ ) - q->location_;
      ...
      if( g && inlier ) {
        vnl_matrix_fixed< double, dimen, 1 >  eCij_1 = Cij_1 * err;
        vnl_matrix_fixed< double, dimen, dimen >  dCpdth = 2.0 * p->covariance_ * Jp.transpose() * eCij_1 * eCij_1.transpose();

        vnl_matrix_fixed< double, dof, 1 >  correct( 0.0 );
        vnl_matrix_fixed< double, dof, 1 >  correctS( 0.0 );
        for( unsigned int f = 0; f < dof-dimen; ++f ) {
          vnl_matrix_fixed< double, dimen, dimen >  Mkl = Mkls[f];

          vnl_matrix_fixed< double, dimen, dimen >  dCijdth = Jp * p->covariance_ * Mkl.transpose() + Mkl * p->covariance_ * Jp.transpose();
          vnl_matrix_fixed< double, 1, 1 >  dCpdth = eCij_1.transpose() * dCijdth * eCij_1;
        ...
        vnl_matrix_fixed< double, dof, 1 >  grad_inc = w * ( rho_p_residual * ( Jth.transpose() * Cij_1 * err * 2.0 + correct ) + correctS );
...
template < unsigned int dimen, unsigned int dof >
void cdcl_obj_fun_par_transfer<dimen, dof>::compute_inverse_gradient( const vnl_vector< double > &  x,
                                                                      double*                       f,
                                                                      vnl_vector< double >*         g,
                                                                      trans_sptr_type      const &  forward )
{
  ...
  vnl_matrix_fixed< double, dimen, dimen > const &  A = trans_->get_A();
  ...
  for( typename vcl_vector< match_sptr_type >::size_type  i = 0; i < matches_.size(); ++i ) {
    ...
    // Jacobian w.r.t. transformation parameters
    vnl_vector_fixed< double, dimen >   translation = forward->get_translation();
    vnl_matrix_fixed< double, dimen, dof > const &  Jth_inv = trans_->jacobian_wrt_inv_par( p->location_ - translation );
    ...
    for( typename vcl_vector< match_sptr_type >::size_type  j = 0; j < matches_[i]->to_.size(); ++j ) {
      ...
      if( g && inlier ) {
        ...
        for( unsigned int f = 0; f < dof-dimen; ++f ) {
          vnl_matrix_fixed< double, dimen, dimen >  Mkl = Mkls[f];
          vnl_matrix_fixed< double, dimen, dimen >  temp_kl = - A * Mkl * A;

          // correction coming from ln det Cij
          vnl_matrix_fixed< double, dimen, dimen >  dCijdth = Jp * p->covariance_ * temp_kl.transpose() + temp_kl * p->covariance_ * Jp.transpose();
          correctS( f, 0 ) = vnl_trace( Cij_1 * dCijdth );
        ...
        vnl_matrix_fixed< double, dof, 1 >  grad_inc = w * ( rho_p_residual * ( Jth_inv.transpose() * Cij_1 * err * 2.0 + correct ) + correctS );
        *g += grad_inc.get_column( 0 );
...
#define CDCL_OBJ_FUN_PAR_TRANSFER_INSTANTIATE( dimen, dof ) \
template class cdcl_obj_fun_par_transfer< dimen, dof >;

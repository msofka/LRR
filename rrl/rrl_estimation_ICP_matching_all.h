#ifndef rrl_estimation_ICP_matching_all_h_
#define rrl_estimation_ICP_matching_all_h_
...
class rrl_estimation_symmetric_ICP_matching_all;


template < unsigned int dim, unsigned int dof >
class rrl_estimation_ICP_matching_all : public cdcl_estimation_abs< dim, dof, cdcl_feature_ICP< dim > >
{
public:

  typedef vbl_smart_ptr< rrl_estimation_ICP_matching_all >  sptr;
  ...
  template < unsigned int dim_other2, unsigned int dof_other2 >
  friend class rrl_estimation_symmetric_ICP_matching_all;

  typedef cdcl_estimation_abs< dim, dof, cdcl_feature_ICP< dim > >  superclass_type;
  ...
  rrl_estimation_ICP_matching_all(): normalize_matches_( false ), weight_by_strength_( false ) {}

  // Construct estimation object given moving and fixed feature sets
  // and an initial transformation.
  rrl_estimation_ICP_matching_all( vcl_vector< feature_sptr_type > const &  moving, 
  ...
  rrl_estimation_ICP_matching_all( vcl_vector< vcl_vector< feature_sptr_type > > const &  moving, 
  ...
  void set_fixed_voronoi_map( VoronoiMapImageType::Pointer const &  fixed_voronoi_map ) { fixed_voronoi_map_ = fixed_voronoi_map; }
...
protected:
  ...
  // Distance map from fixed features for fast matching.
  VoronoiMapImageType::Pointer  fixed_voronoi_map_;
